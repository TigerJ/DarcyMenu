...
ig.baked=true;ig.module('plugins.darcy').requires('impact.impact','impact.system').defines(function(){ig.Darcy=ig.Class.extend({MenuItems:new Object([]),cFillStyle:"",Font:"",Stroke:false,StrokeType:"",Corner:0,ButtonSize:{x:0,y:0},Spacing:{x:0,y:0},MenuStart:{x:0,y:0},MenuLayout:"",init:function(){},update:function(){for(i=0;i<this.MenuItems.length;i++)
{var leftBounds=this.MenuStart.x+((this.ButtonSize.x+this.Spacing.x)*(i));var rightBounds=this.MenuStart.x+((this.ButtonSize.x+this.Spacing.x)*(i))+this.ButtonSize.x;var upperBounds=this.MenuStart.y+((this.ButtonSize.y+this.Spacing.y)*(i));var lowerBounds=this.MenuStart.y+((this.ButtonSize.y+this.Spacing.y)*(i))+this.ButtonSize.y;if(ig.input.mouse.x>leftBounds&&ig.input.mouse.x<rightBounds&&ig.input.mouse.y>upperBounds&&ig.input.mouse.y<lowerBounds)
{if(ig.input.pressed(this.MenuItems[i].inputAction))
{this.MenuItems[i].inputEvent.action();}}}},draw:function(){if(this.isImpactfont)
{}
else
{for(i=0;i<this.MenuItems.length;i++)
{ig.system.context.textBaseline='top';ig.system.context.fillStyle=this.cFillStyle;ig.system.context.font=this.Font;if(this.MenuLayout=="Horizontal")
{ig.system.context.fillText(this.MenuItems[i].text,this.MenuStart.x+((this.ButtonSize.x+this.Spacing.x)*(i)),this.MenuStart.y);}
if(this.MenuLayout=="Verticle")
{ig.system.context.fillText(this.MenuItems[i].text,this.MenuStart.x,this.MenuStart.y+((this.ButtonSize.y+this.Spacing.y)*(i)));}}}},addItem:function(menutext,menulabel,inputAction,inputEvent){var item=new Object;item.text=menutext;item.label=menulabel;item.inputAction=inputAction;item.inputEvent=inputEvent;this.MenuItems.push(item);},removeItem:function(label)
{}});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','plugins.darcy').defines(function(){MyGame=ig.Game.extend({font:new ig.Font('media/04b03.font.png'),darcy:new ig.Darcy(),init:function(){ig.input.bind(ig.KEY.MOUSE1,'shoot');this.darcy.cFillStyle="white";this.darcy.Font="14pt arial";this.darcy.Spacing.x=10;this.darcy.MenuStart.x=10;this.darcy.ButtonSize.x=100;this.darcy.ButtonSize.y=30;this.darcy.MenuLayout="Verticle";this.darcy.Spacing.y=10;this.darcy.addItem("polaris","cat","shoot",{action:function(){console.log("black cat");}});this.darcy.addItem("voilett","dog","shoot",{action:function(){console.log("german shepard");}});this.darcy.addItem("atticus","horse");this.darcy.addItem("marvin","placo");},update:function(){this.darcy.update();this.parent();},draw:function(){this.parent();var x=ig.system.width/2,y=ig.system.height/2;this.font.draw('It Works!',x,y,ig.Font.ALIGN.CENTER);this.darcy.draw();}});ig.main('#canvas',MyGame,60,320,240,2);});