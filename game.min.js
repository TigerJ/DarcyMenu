...
ig.baked=true;ig.module('plugins.darcy').requires('impact.impact','impact.system').defines(function(){ig.Darcy=ig.Class.extend({MenuItems:[],ButtonProperties:{Font:"",TextAlign:"center",Stroke:false,StrokeType:"",Corner:0,ButtonSize:{x:0,y:0},Spacing:{x:0,y:0},DrawBoxes:false,BoxStrokeStyle:null,BoxStrokeWidth:null,cFillStyle:""},MenuStart:{x:0,y:0},MenuLayout:"",init:function(ButtonProperties){ig.merge(this.ButtonProperties,ButtonProperties);},update:function(){this.updateMenuItems();},updateMenuItems:function()
{for(i=0;i<this.MenuItems.length;i++)
{this.MenuItems[i].update();}},draw:function(){if(this.isImpactfont)
{}
else
{for(i=0;i<this.MenuItems.length;i++)
{ig.system.context.fillStyle=this.ButtonProperties.cFillStyle;ig.system.context.textAlign=this.ButtonProperties.TextAlign;ig.system.context.font=this.ButtonProperties.Font;if(this.DrawBoxes)
...
{this.drawItems();}}}},drawItems:function()
{for(i=0;i<this.MenuItems.length;i++)
{this.MenuItems[i].draw();}},addItem:function(menutext,menulabel,inputAction,inputEvent){var dItem=new ig.DarcyItem();dItem.text=menutext;dItem.label=menulabel;dItem.inputAction=inputAction;dItem.inputEvent=inputEvent;dItem.drawBox=this.ButtonProperties.DrawBoxes;dItem.boxStrokeStyle=this.ButtonProperties.BoxStrokeStyle;dItem.boxStrokeWidth=this.ButtonProperties.BoxStrokeWidth;dItem.CornerRad=this.ButtonProperties.Corner;dItem.MenuLayout=this.MenuLayout;if(this.MenuLayout=="V")
...
{}},draw:function(){if(this.MenuLayout=="V")
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','plugins.darcy').defines(function(){MyGame=ig.Game.extend({font:new ig.Font('media/04b03.font.png'),darcy:new ig.Darcy(),darcy2:new ig.Darcy(),init:function(){ig.input.bind(ig.KEY.MOUSE1,'shoot');this.darcy.ButtonProperties={cFillStyle:"red",Font:"14pt arial",TextAlign:"center",Spacing:{x:5,y:5},ButtonSize:{x:100,y:30},DrawBoxes:true,BoxStrokeStyle:"#fff",BoxStrokeWidth:2,Corner:5};this.darcy.MenuLayout="H";this.darcy.MenuStart.x=5;this.darcy.MenuStart.y=5;this.darcy.addItem("voilett","dog","shoot",function(){console.log("german shepard dog");});this.darcy.addItem("polaris","cat","shoot",function(){console.log("black cat");});this.darcy.addItem("atticus","horse","shoot",function(){console.log("horse");});this.darcy2.ButtonProperties={cFillStyle:"red",Font:"12pt serif",TextAlign:"center",Spacing:{x:5,y:5},ButtonSize:{x:100,y:20},DrawBoxes:true,BoxStrokeStyle:"#fff",BoxStrokeWidth:2};this.darcy2.MenuLayout="V";this.darcy2.MenuStart.x=5;this.darcy2.MenuStart.y=40;this.darcy2.addItem("voilett","dog","shoot",function(){console.log("german shepard dog");});this.darcy2.addItem("polaris","cat","shoot",function(){console.log("black cat");});this.darcy2.addItem("atticus","horse","shoot",function(){console.log("horse");});},update:function(){this.darcy.update();this.darcy2.update();this.parent();},draw:function(){this.parent();var x=ig.system.width/2,y=ig.system.height/2;this.font.draw('It Works!',x,y,ig.Font.ALIGN.CENTER);this.darcy.draw();this.darcy2.draw();}});ig.main('#canvas',MyGame,60,320,240,2);});